...
| `L0424` | semantic | Unsupported `export fn` (C-callable export) signature. | Exposing a Lullaby function to C currently covers only the Win64 integer convention, so every parameter and the return type must be `i64` and the function must not be generic. | Restrict the exported function to `i64` parameters and an `i64` return type, or remove the `export` marker. |

...
| :--- | :--- | :--- | :--- | :--- |
| **1.1** | **Lexer & Parser Implementation:** Implement the core components to read raw lullaby source code and convert it into an Abstract Syntax Tree (AST). | `lullaby_syntax_design.md` | High | Alpha functions/expressions/arrays/conditionals/loops/range-for done; formal Alpha 1 grammar draft in `formal_grammar.md`; planned syntax keywords now reject with `L0211`; ongoing for full language |
...
| **6.6** | **Diagnostic Registry:** Document every stable `L####` code with meaning, likely cause, example, and suggested fix; keep Markdown and offline browser docs synchronized. | 6.1 | Medium | Alpha registry done in `diagnostic_registry.md` and offline docs |
| **6.7** | **Diagnostic Snapshot Tests:** Add representative CLI tests for lexer, parser, semantic, optimizer, bytecode/artifact, runtime, and resource diagnostics in concise, verbose, and JSON modes. | 6.2, 6.3 | High | Alpha CLI diagnostics coverage done, including `L0211` planned-syntax rejection and `L0329` executable entry-point validation |
| **6.8** | **IR Diagnostics:** Report IR lowering failures through the shared diagnostic model with phase, code, span, source path, and JSON/verbose rendering support. | 3.1, 6.1 | Medium | Alpha `L0501` IR lowering diagnostic done |
| **6.9** | **Bytecode Artifact Diagnostics:** Report malformed or unsupported compiled artifacts through the shared diagnostic model with phase, code, path, and JSON/verbose rendering support. | 3.1, 6.1 | Medium | Alpha `L0601` bytecode artifact diagnostic done |

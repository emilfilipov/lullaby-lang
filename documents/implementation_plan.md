...
- `nous_ir`: typed semantic IR schema, lowering from `CheckedProgram`, deterministic optimization pass configuration with initial constant folding, block-local common subexpression elimination, conservative loop-invariant motion, copy propagation, and dead-code elimination, executable IR interpreter, explicit instruction-bytecode lowering, versioned `.nbc` artifact encoding/decoding with metadata/function-table compatibility checks, and bytecode VM entry point for the current alpha subset.
- `nous_cli`: exposes the `nlang` binary with `check <file.nl>`, `compile [--optimize none|constant-fold|dead-code|alpha] [-o output.nbc] <file.nl>`, `build [--optimize none|constant-fold|dead-code|alpha] [-o output.nbc] <file.nl>`, `run [--backend ast|ir|bytecode] [--optimize none|constant-fold|dead-code|alpha] <file.nl|file.nbc>`, and `docs`.
...
| :--- | :--- | :--- | :--- | :--- |
| **1.1** | **Lexer & Parser Implementation:** Implement the core components to read raw nlang source code and convert it into an Abstract Syntax Tree (AST). | `nous_lang_syntax_design.md` | High | Alpha functions/expressions/arrays/conditionals/loops/range-for done; formal Alpha 1 grammar draft in `formal_grammar.md`; planned syntax keywords now reject with `N0211`; ongoing for full language |
...
| Story | Description | Dependencies | Estimated Effort | Status |
| :--- | :--- | :--- | :--- | :--- |
| **3.1** | **Compiler Toolchain:** Implement the full compiler pipeline defined in `nous_lang_compilation_architecture.md` to handle source code compilation into machine-readable bytecode or an intermediate representation. | All Runtime Components | High | Typed semantic IR lowering, opt-in constant folding, block-local CSE, conservative loop-invariant motion, copy propagation, and dead-code elimination, IR interpreter, CLI backend selection, `nlang compile`, versioned `.nbc` artifacts with metadata/function-table compatibility checks, and explicit instruction-bytecode module execution done for current alpha subset; broader backend/native work pending |
...
| Story | Description | Dependencies | Estimated Effort | Status |
| :--- | :--- | :--- | :--- | :--- |
| **4.1** | **CLI Tool Implementation:** Implement the command-line interface (CLI) tool that allows users to invoke the compiled nlang executable (`nlang run script.nl`). | 3.3 | Medium | Initial `check`, `compile`, `build`, `inspect`, `run`, `docs`, `examples`, `help`, and `--version` commands done; `check` supports helper/library-style validation, `compile`/`build` and source `run` require zero-argument `main`, `run` supports source backends plus compiled `.nbc` artifacts, and artifact generation plus IR/bytecode source runs support opt-in optimization |
...
| **5.2** | **Component Unit Testing:** Implement unit tests for each major component: Lexer (tokenization), Parser (AST generation), Memory Manager (allocation/deallocation), and Type Checker. | Stories 1.1, 1.2, 1.3 | High | In Progress; parser AST golden snapshot workflow is in place under `crates/nous_parser/tests/snapshots/` |

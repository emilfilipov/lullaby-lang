...
**Goal:** Create a complete toolchain and installer for Lullaby (lullaby) that allows users to compile and run `.lullaby` source files easily via the command line.
...
- `lullaby_lexer`: validates `.lullaby` source paths, tokenizes source text, emits indentation/dedent tokens, recognizes the current core keywords, and rejects curly braces and semicolon terminators.
...
- `lullaby_ir`: typed semantic IR schema, lowering from `CheckedProgram`, memory-operation analysis for Alpha 1 allocation/load/store/deallocation and bounds-checked indexing, deterministic optimization pass configuration with initial constant folding, block-local common subexpression elimination, conservative loop-invariant motion, copy propagation, and dead-code elimination, executable IR interpreter, explicit instruction-bytecode lowering, versioned `.lbc` artifact encoding/decoding with metadata/function-table compatibility checks, and bytecode VM entry point for the current alpha subset.
- `lullaby_cli`: exposes the `lullaby` binary with `check <file.lullaby>`, `compile [--optimize none|constant-fold|dead-code|alpha] [-o output.lbc] <file.lullaby>`, `build [--optimize none|constant-fold|dead-code|alpha] [-o output.lbc] <file.lullaby>`, `run [--backend ast|ir|bytecode] [--optimize none|constant-fold|dead-code|alpha] <file.lullaby|file.lbc>`, and `docs`.
- `tests/fixtures`: valid and invalid `.lullaby` fixtures for frontend, semantic, runtime, and CLI smoke checks.
...
- `examples`: user-facing valid and invalid `.lullaby` programs packaged with the toolchain.
- `scripts`: Windows portable package, cross-platform portable package driver, optional PATH helper, GitHub release publication, and release verification scripts that build `bin\lullaby.exe`, bundle offline docs and examples, create an archive plus checksum, and smoke-test the unpacked toolchain.
...
| **1.5.5** | **Offline Docs Verification:** Add checks that verify the generated HTML entry point exists, local links resolve, required sections are present, and the bundle can be opened from disk without missing assets. | 1.5.2, 1.5.3 | Medium | Initial shipped-doc verifier done; now also validates executable docs examples against fixtures and verifies generated docs with `--profile generated`, including generated quick start, CLI, package layout, diagnostics, and limitations sections |
| **1.5.6** | **Installer Bundle Integration:** Prepare the offline docs output for packaging with the future toolchain installer and document where the installer should place the local entry page. | Epic 3, Epic 4 | Medium | Alpha portable package bundles `docs\index.html`; `lullaby docs` locates the local entry page |
...
| Story | Description | Dependencies | Estimated Effort | Status |
| :--- | :--- | :--- | :--- | :--- |
| **3.1** | **Compiler Toolchain:** Implement the full compiler pipeline defined in `lullaby_compilation_architecture.md` to handle source code compilation into machine-readable bytecode or an intermediate representation. | All Runtime Components | High | Typed semantic IR lowering, Alpha 1 memory-operation analysis, opt-in constant folding, block-local CSE, conservative loop-invariant motion, copy propagation, and dead-code elimination, IR interpreter, CLI backend selection, `lullaby compile`, versioned `.lbc` artifacts with metadata/function-table compatibility checks, and explicit instruction-bytecode module execution done for current alpha subset; broader backend/native work pending |
| **3.2** | **Build Script Generation:** Create a robust, platform-agnostic build script (e.g., using CMake or a custom script) that orchestrates the compilation of the compiler and runtime into a single binary. | 3.1 | Medium | Alpha Windows release build path done through Cargo plus `scripts/package_windows_portable.ps1`; initial cross-platform portable package driver done in `scripts/package_portable.py`; host-native GitHub Actions workflow template added in `portable_package_ci_workflow.yml`; workflow activation and explicit target-triple package jobs pending |
| **3.3** | **Installer Creation:** Develop the installer logic to bundle the compiled lullaby executable, necessary libraries, and documentation into a single user-friendly package (e.g., .exe or system package). | 3.2 | High | Alpha Windows portable zip package done with optional user PATH install/uninstall helpers and SHA-256 checksum; generated-docs portable archive path done in `scripts/package_portable.py`; full platform-specific installers pending |

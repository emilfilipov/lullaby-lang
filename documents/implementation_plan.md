...
| Story | Description | Dependencies | Estimated Effort | Status |
| :--- | :--- | :--- | :--- | :--- |
| **3.1** | **Compiler Toolchain:** Implement the full compiler pipeline defined in `lullaby_compilation_architecture.md` to handle source code compilation into machine-readable bytecode or an intermediate representation. | All Runtime Components | High | Typed semantic IR lowering, Alpha 1 memory-operation analysis, opt-in constant folding, block-local CSE, conservative loop-invariant motion, copy propagation, and dead-code elimination, IR interpreter, CLI backend selection, `lullaby compile`, versioned `.lbc` artifacts with metadata/function-table/memory-operation compatibility checks, `lullaby inspect` memory metadata reporting, and explicit instruction-bytecode module execution done for current alpha subset; broader backend/native work pending |

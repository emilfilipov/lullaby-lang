...
Within otherwise-`i64` functions, the native backend also compiles the fixed-width integer types (`i8`/`i16`/`i32`/`u8`/`u16`/`u32`/`u64`/`isize`/`usize`). Each fixed-width value is held in a 64-bit register as the same normalized `i64` cell the interpreters use (signed kinds sign-extended, unsigned zero-extended, the 64-bit kinds filling the cell). Scalar `i64` functions get **register promotion**: two `i64` locals are kept in callee-saved registers (`rbx`/`rsi`) instead of stack slots, saved/restored across calls. The two are chosen by **loop-weighted usage** — a use inside a loop counts `8×` more per nesting level — so a hot loop counter/accumulator wins promotion over a loop-invariant parameter (correctness is independent of the choice; only speed changes). This covers functions containing range `for` loops — the loop counter and body accumulators live in registers; the loop's hidden bound/step stay as loop-invariant stack operands. It also unlocks the **runtime-bound counting-sum ILP unroll**: `while i < n` (an `i64` variable bound, not just a constant) now folds four iterations per step like the constant-bound case, because the counter and accumulator are promoted ahead of `n` — turning a loop that was ~5× slower than C into one that matches/beats it. The reduction unroll generalizes past the bare counting sum: an **affine** addend `acc += a*i + b` (`i+i`, `3*i+5`, weighted sums) folds four iterations into one `imul`+`add` closed block sum (`acc += 4a*i + (6a+4b)`), running **~1.8× faster than C**; a non-affine polynomial addend (`acc += i*i`) uses a four-way **multi-accumulator** unroll that breaks the serial `acc` dependency chain (~1.7× faster than the naive scalar loop). All three are bit-identical to the scalar loop under wrapping arithmetic and are guarded by the differential fuzzer.

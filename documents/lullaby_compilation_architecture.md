...
The native backend also **auto-vectorizes** the canonical sum reduction `for i from S to E: acc += a[i]` over a contiguous `array<i64>`: it emits an SSE2 packed loop (`pxor` to zero a 128-bit accumulator, `movdqu`+`paddq` to add two `i64` elements per iteration), horizontal-adds the two lanes into `acc`, then a scalar tail handles a final odd element. Because integer addition is associative and commutative mod 2^64, the total is bit-for-bit identical to the scalar loop (including overflow wrap); measured ~3.3x faster on a large array sum. The pattern matcher is exact — any other loop shape (a multiplied index, a different body, a non-unit step) falls through to the scalar `for` lowering, so correctness never depends on it. Dot-product and element-wise map loops are the natural next patterns.

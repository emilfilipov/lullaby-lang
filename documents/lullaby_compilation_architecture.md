...
Within otherwise-`i64` functions, the native backend also compiles the fixed-width integer types (`i8`/`i16`/`i32`/`u8`/`u16`/`u32`/`u64`/`isize`/`usize`). Each fixed-width value is held in a 64-bit register as the same normalized `i64` cell the interpreters use (signed kinds sign-extended, unsigned zero-extended, the 64-bit kinds filling the cell). Scalar `i64` functions get **register promotion**: their `i64` locals are kept in callee-saved registers (`rbx`/`rsi`) instead of stack slots, saved/restored across calls. This now covers functions containing range `for` loops (previously excluded entirely) — the loop counter and body accumulators live in registers, so a tight counting loop like `for i … s += …` runs about 10% faster; the loop's hidden bound/step stay as loop-invariant stack operands.

The native backend also **auto-vectorizes** two array loop shapes over contiguous `array<i64>`. **Sum reduction** `for i from S to E: acc += a[i]` emits an SSE2 packed loop (`pxor` to zero a 128-bit accumulator, `movdqu`+`paddq` to add two `i64` elements per iteration), horizontal-adds the two lanes into `acc`, then a scalar tail handles a final odd element; measured ~3.3x faster on a large array sum. **Element-wise map** `for i from S to E: c[i] = a[i] + b[i]` (or `-`) emits a packed load/add/store (`movdqu` two lanes from each source, `paddq`/`psubq`, `movdqu` the result), again with a scalar tail; measured 3.36x faster than the same loop forced scalar. Because integer add/sub is associative mod 2^64 and the lanes are independent, both are bit-for-bit identical to the scalar loop (including overflow wrap) and are parity-tested against every interpreter. The pattern matcher is exact — any other loop shape (a multiplied index, a different body, a non-unit step, a mismatched destination) falls through to the scalar `for` lowering, so correctness never depends on it. Min/max/count/product reductions, `f64` accumulation, and the dot product are the natural next patterns.

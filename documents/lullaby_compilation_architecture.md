...
`lullaby_ir::native_object` now provides the first object-emission prototype for `x86_64-pc-windows-msvc`. It emits a deterministic COFF object for a zero-argument `main` that returns a literal `i64`, literal `bool`, `void`, or a stack-backed `i64` local arithmetic expression, after the source has already passed semantic validation, typed IR lowering, and bytecode lowering. Broader instruction lowering, linker orchestration, and native runtime packaging remain planned work.

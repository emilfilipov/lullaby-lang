...
The **same value-copy rule applies to local `let` bindings and assignments**, not just call boundaries: binding or assigning a mutable-aggregate **lvalue** — `let g = f`, `g = f`, or a field/element store `o.inner = q` where the RHS is a variable / field / index read of a `struct`/`array`/`enum`/`list`/`map` — **deep-copies** the value (reusing the same `emit_deep_copy` machinery) so the binding is an independent record, exactly as the three interpreters clone the value. A **freshly constructed** RHS (a struct/enum construction, an array literal, or a call result — each already allocates its own record) is bound directly with no redundant copy, matching the interpreters, which clone only when reading an lvalue. This closes a real cross-tier miscompile: previously `let g = f` bound `f`'s linear-memory pointer, so a later `g.a = …` also mutated `f` (WASM alone returned the corrupted value; the three interpreters and native returned the correct one). The fix is verified by the in-process **`wasmi` execution-parity suite** (`crates/lullaby_ir/src/wasm_exec_tests.rs`), which runs the emitted module and asserts `main`'s `i64` return equals the IR interpreter's for every observable copy shape (struct/array `let` and `=`, nested array-field copy, aggregate field-store, a by-value parameter copied inside a callee, and a returned struct copied) — the WASM analogue of the native compile-and-run exit-code tests. Enums and `list`/`map` have no in-place payload mutation (their "mutations" construct fresh values), so their `let`-aliasing was never observable; they are deep-copied anyway for consistency and covered by parity guards.

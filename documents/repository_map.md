...
- `cargo test -p lullaby_ir native_object`: verify the first Alpha 1 native object emitter for minimal COFF output and unsupported-body errors.
- `cargo test -p lullaby_ir --test native_object_snapshots`: verify the checked-in literal-return COFF object-emission snapshot.

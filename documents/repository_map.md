...
- `crates/lullaby_lexer/`: source extension validation, tokenization, indentation scanning, string- and char-literal lexing (single-quoted `'c'` char literals, with malformed-literal diagnostic `L0105`), forbidden brace/semicolon diagnostics, current and planned keyword recognition, and lexical tests.
...
- `crates/lullaby_parser/`: AST model and parser for function declarations, typed parameters, return types, indentation blocks, explicit and inferred `let`, assignment, `return`, `break`, `continue`, if/elif/else, while/loop/range-for, calls, integer and `f64` float literals, `'c'` char literals, array literals/indexing, struct declarations/construction/field access, enum (tagged-union) declarations with positional payload types, `match` expressions over enums with variant/wildcard patterns and payload bindings, variables, arithmetic, comparison, logical expressions, and `L0211` rejection for planned syntax keywords that are not in Alpha 1. Also hosts `format.rs`, the canonical source formatter (`format_program`) that renders an AST back to idempotent, source-ordered `.lby` source and backs `lullaby fmt`.
...
- `cargo run -p lullaby_cli -- check tests/fixtures/invalid/unsupported_import.lby`: verify `L0211` planned-syntax diagnostics.

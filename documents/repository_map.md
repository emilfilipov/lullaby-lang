...
- `documents/option_result_design.md`: `option<T>`/`result<T, E>` design as built-in generic enums — variants `some`/`none`/`ok`/`err`, context-directed type inference at `let`/return/`match` sites, the multi-argument generic-type foundation (`generic_args`, comma-separated generic spelling), backend-parity representation via `Value::Enum`, and `L0386`.
...
- `documents/generics_design.md`: user-defined generics design — generic functions `fn name<T> ...` with call-site type inference, erased at runtime (backends unchanged), inference mirrored in the IR lowerer, and `L0394`/`L0395`/`L0396`; generic user types, explicit type arguments, and trait bounds deferred to the trait work.
- `documents/traits_design.md`: traits/interfaces design — `trait Name` method signatures with a `self` receiver, `impl Trait for Type`, receiver-type method dispatch via a shared impl table on all backends, and trait bounds on generics (`<T: Ord>`); records `L0397`/`L0398`/`L0399` and defers default bodies, trait objects, associated types, and supertraits.
...
- `crates/lullaby_parser/`: AST model and parser for function declarations, typed parameters, return types, indentation blocks, explicit and inferred `let`, assignment, `return`, `break`, `continue`, if/elif/else, while/loop/range-for, calls, integer and `f64` float literals, `'c'` char literals, array literals/indexing, struct declarations/construction/field access, enum (tagged-union) declarations with positional payload types, `match` expressions over enums with variant/wildcard patterns and payload bindings, variables, arithmetic, comparison, logical expressions, leading `import NAME` module lines and an optional `pub` export modifier on top-level `fn`/`struct`/`enum`/`alias` declarations (`imports`/`is_public` are serde-defaulted so single-file artifacts and AST snapshots stay valid), and `L0211` rejection for planned syntax keywords that are not in Alpha 1. Also hosts `format.rs`, the canonical source formatter (`format_program`) that renders an AST back to idempotent, source-ordered `.lby` source and backs `lullaby fmt`.
...
- `crates/lullaby_runtime/`: in-process AST runtime for the current alpha subset, including `main`, function calls, scoped locals, assignment, branch results, while/loop/range-for execution, break/continue, array literals/indexing with runtime bounds checks, struct and enum (tagged-union) value construction, arithmetic/comparison/logical expressions with short-circuiting, `alloc`/`load`/`store`/`dealloc` heap-slot memory builtins, reference-counted `rc_new`/`rc_clone`/`rc_release`/`rc_get`/`rc_borrow`/`ref_get` builtins with a refcount table plus `unsafe`-gated raw `ptr_read`/`ptr_write`, text file I/O builtins, standard stream builtins (`print`/`println`/`warn`/`flush`), direct program-plus-argv system command builtins, `throw`/`try`/`catch` structured error recovery for user-thrown errors, and categorized runtime/resource errors.
- `crates/lullaby_cli/src/loader.rs`: multi-file module loader. `load_program(entry)` lexes+parses the entry file and every transitively imported `NAME.lby` sibling, detects import cycles (`L0393`) and missing module files (`L0394`), enforces the flat-namespace no-shadowing rule (`L0391`) and cross-module `pub` visibility (`L0392`) before semantics, and merges all modules into one flat `Program`. Backends and the semantic analyzer are unchanged; a single-file program with no imports behaves as a direct lex+parse.

...
- `crates/lullaby_parser/`: AST model and parser for function declarations, typed parameters, return types, indentation blocks, explicit and inferred `let`, assignment, `return`, `break`, `continue`, if/elif/else, while/loop/range-for, calls, integer and `f64` float literals, array literals/indexing, struct declarations/construction/field access, variables, arithmetic, comparison, logical expressions, and `N0211` rejection for planned syntax keywords that are not in Alpha 1.

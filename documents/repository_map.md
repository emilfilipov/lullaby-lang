...
- `documents/enum_and_match_design.md`: token-minimalist enum/tagged-union and `match` design — indentation-only `enum` declaration with positional payload types, `Enum.Variant` construction, exhaustive `match` with payload binding and wildcard, nominal typing, backend-parity representation, and sequencing toward `option`/`result`.

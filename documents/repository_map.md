...
- `crates/lullaby_parser/`: AST model and parser for function declarations, typed parameters, return types, indentation blocks, explicit and inferred `let`, assignment, `return`, `break`, `continue`, if/elif/else, while/loop/range-for, calls, integer and `f64` float literals, `'c'` char literals, array literals/indexing, struct declarations/construction/field access, enum (tagged-union) declarations with positional payload types, `match` expressions over enums with variant/wildcard patterns and payload bindings, variables, arithmetic, comparison, logical expressions, leading `import NAME` module lines and an optional `pub` export modifier on top-level `fn`/`struct`/`enum`/`alias` declarations, an optional `<T, U>` generic type-parameter list after a function name with duplicate/built-in-shadowing rejection (`L0394`) (`imports`/`is_public`/`type_params` are serde-defaulted so single-file artifacts and AST snapshots stay valid), and `L0211` rejection for planned syntax keywords that are not in Alpha 1. Also hosts `format.rs`, the canonical source formatter (`format_program`) that renders an AST back to idempotent, source-ordered `.lby` source and backs `lullaby fmt`.

...
- `crates/lullaby_ir/`: typed semantic IR schema, lowering from `CheckedProgram`, Alpha 1 memory-operation analysis with safety metadata for allocation/load/store/deallocation and bounds-checked indexing, deterministic optimization pass configuration with opt-in constant folding, conservative block-local CSE, conservative loop-invariant motion, conservative block-local copy propagation, block-local dead-code elimination, executable IR interpreter, inline and fixture-driven AST/IR/bytecode parity tests including optimized backend variants, explicit instruction-bytecode lowering, versioned `.lbc` artifact encoding/decoding with metadata/function-table/instruction-contract compatibility checks, and bytecode VM entry point for the current alpha subset.
...
- `cargo run -p lullaby_cli -- inspect target/run_arithmetic.lbc`: print `.lbc` artifact metadata and function signatures without executing it.

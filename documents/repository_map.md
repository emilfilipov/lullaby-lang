...
- `cargo test -p lullaby_ir native_object`: verify the first Alpha 1 native object emitter for minimal COFF output, stack-backed `i64` local arithmetic, and unsupported-body errors.
- `cargo test -p lullaby_ir --test native_object_snapshots`: verify the checked-in literal-return and local-arithmetic COFF object-emission snapshots.

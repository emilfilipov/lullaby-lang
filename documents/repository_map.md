...
- `documents/struct_design.md`: token-minimalist struct/record design — indentation-only `struct` declaration with `name type` fields, positional call-style construction, `.` field access, nominal typing, and the first-increment scope versus deferred features.
...
- `crates/lullaby_parser/`: AST model and parser for function declarations, typed parameters, return types, indentation blocks, explicit and inferred `let`, assignment, `return`, `break`, `continue`, if/elif/else, while/loop/range-for, calls, integer and `f64` float literals, array literals/indexing, struct declarations/construction/field access, variables, arithmetic, comparison, logical expressions, and `L0211` rejection for planned syntax keywords that are not in Alpha 1.
...
- `crates/lullaby_semantics/`: static validation for duplicate declarations, explicit and inferred local binding types, assignment targets/types, function call arity/types, return behavior, bool conditions, loop-control placement, arithmetic/comparison/logical expression operand types, homogeneous non-empty arrays, array indexing, nominal struct types with positional construction and field access, interim pointer-style memory builtins, reference-counted `rc<T>`/`ref<T>`/`ptr<T>` types with `rc_new`/`rc_clone`/`rc_release`/`rc_get`/`rc_borrow`/`ref_get` builtins and `unsafe`-gated raw `ptr_read`/`ptr_write`, text file I/O builtins, standard stream builtins (`print`/`println`/`warn`/`flush`), `i64`/`f64` numeric arithmetic, `to_string` conversion and `+` string concatenation, safe system command builtins, and executable zero-argument `main` validation for compile/run. Successful validation returns `CheckedProgram` with function signatures and inferred expression-type metadata.

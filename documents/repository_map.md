...
- `scripts/package_windows_portable.ps1`: builds the release CLI, creates `dist\nous-lang-alpha1-windows-x64\`, copies `bin\nlang.exe`, bundles `docs\index.html`, release notes, and valid examples, copies a repository license file if one exists, writes package metadata/readme files, and creates a `.zip` archive.
...
- `cargo run -p nous_cli -- compile --optimize alpha -o target/run_arithmetic.nbc tests/fixtures/valid/run_arithmetic.nl`: compile a valid source fixture into a versioned `.nbc` bytecode artifact with metadata and a function table.
...
- `cargo run -p nous_cli -- run --backend bytecode --optimize constant-fold tests/fixtures/valid/run_logic.nl`: run a valid fixture through typed IR lowering, the opt-in constant-folding pass, structured bytecode lowering, and the bytecode VM entry point.
...
- `cargo run -p nous_cli -- run --backend bytecode --optimize alpha tests/fixtures/valid/run_arithmetic.nl`: run a valid fixture through typed IR lowering, the current alpha optimizer pipeline, structured bytecode lowering, and the bytecode VM entry point.

...
- `dealloc(ptr)` clears a heap slot and reports a runtime error on invalid or double deallocation. Note the free tracking is layered:
  - **Statically (`L0350`)**: a straight-line use-after-free / double-free, **including through a direct binding-to-binding copy** — `let q = p` / `dealloc(p)` / `ptr_read(q)` is rejected at compile time, as is the double free `dealloc(p)` / `dealloc(q)`. Copy-aliasing is symmetric and transitive: `let q = p` / `let r = q` puts all three in one group, so freeing any of them kills all of them, and re-binding a name (`q = alloc(1)`) detaches and revives just that name. The walk covers **every expression position in the body**, not just statement values: an inline-`asm` operand clause and an **assignment target's index** are ordinary expressions, so `dealloc(p)` then `a[ptr_read(p)] = 99` is rejected exactly as the hoisted `let i = ptr_read(p)` / `a[i] = 99` form is. (Both were once blind spots — a walk that dropped `Stmt::Assign`'s `path` saw only the right-hand side.)
  - **Still open — caught only at run time (`L0406`)**: an alias the checker cannot see is not tracked, and the checker does not pretend otherwise. This is copy tracking, **not alias analysis**. Not tracked: an alias through a call (`let q = identity(p)`, or a box a helper returns), through an aggregate (a struct field, array element, or `list`), across a function boundary (passing a box to a callee that frees it), and a free inside a branch or loop body (deliberately not escaping its block — conservative, to avoid false positives on a conditional free). Closing these needs interprocedural alias analysis.
  - This layering is why native `dealloc` **skips cleanly** rather than lowering to `rc_free`. `rc_free` is only safe once the aliasing routes above are closed: today a use-after-free through an untracked alias would silently read free-list memory instead of raising `L0406`, and a double free through one would push a block onto the free list twice, making it cyclic. See `native_object_heapbox.rs`.

...
- Structured error handling: `throw EXPR` raises a `string` error value; `try` / `catch NAME` runs a protected block and, if it throws, binds the error message to `NAME` (a `string`) and runs the handler for recovery. Uncaught throws surface as runtime `N0420`. Only user-thrown errors are caught; a `try` without `catch` reports `N0213`. Structured error handling currently runs on the AST backend; the IR/bytecode backends reject it during lowering.
- Type aliases: `alias NAME = TYPE` declares a top-level type alias (e.g. `alias Count = i64`, `alias Numbers = array<i64>`). Aliases resolve structurally to their canonical target before type checking, so an alias and its target are interchangeable, and aliases carry no runtime representation. Aliases inside generic arguments (`array<Count>`) resolve too. Duplicate aliases report `N0360` and cyclic aliases report `N0361`. Struct/record and map generics remain planned until those types exist.
...
- `try`/`catch`/`throw` on the optimizing IR and bytecode backends (structured error handling runs on the default AST backend only).

...
- User-defined generic *types* (`struct Box<T>`, `enum Tree<T>`), **explicit type arguments** at a call site (`identity<i64>(3)`), and **trait bounds** (`<T: Ord>`). User-defined generic *functions* with call-site type inference are implemented (see [generics_design.md](generics_design.md)); bare-`T` arithmetic and ordering wait on trait bounds. Argument-position inference for `option`/`result` construction also remains planned.

...
3. **The two models do not interoperate at a boundary — except through `ptr_cast`,
   ...
   **caller's expected annotation**, defaulting to `ptr<i64>`, and never from the
   ...
   the one conversion the rest of the type system refuses. `ptr_cast` is the **only**
   builtin whose result type ignores its operand (`check_ptr_offset` returns
   `Some(ptr_ty)`, preserving it; `check_addr_of` derives `ptr<T>` from the place;
   `int_to_ptr` takes an `i64`).

...
3. **The two models do not interoperate at a boundary — except through `ptr_cast`,
   ...
   **Semantics-side fix needed (not made here — `crates/lullaby_semantics/**` is
   owned elsewhere).** `check_ptr_cast` should either (a) **preserve the operand's
   model** — a `ptr_T` operand yields a `ptr_T` result, so a box stays a box and the
   `L0303`/`L0313` walls keep holding — or (b) **reject a `ptr_T` operand outright**
   (`ptr_cast` retargets a pointee type, which is meaningless for a one-cell box the
   interpreters address by slot index). (a) is the more faithful fix. Until then the
   **native backend refuses `ptr_cast` over a `ptr_T` operand** (`L0339`), which
   closes the miscompile but is a backend patch for a frontend hole: the
   interpreters still accept the laundered program and merely fail later at run time
   with `L0406`.

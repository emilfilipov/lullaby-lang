...
Deferred beyond this increment: `map<K, V>`; **lists of heap elements** (`list<string>`/`list<struct>`/`list<list<…>>`/`list<map<…>>`) — the element deep-copy would need per-element type dispatch; trapping native bounds checks for `get`/`set`; and a heap `free`/reclamation path (grown/copied blocks are orphaned).

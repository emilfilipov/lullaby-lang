...
- **Now delivered:** the `sqrt(x)` math builtin on `f64` — lowered inline as a single SSE2 `sqrtsd xmm0, xmm0` (`F2 0F 51 C0`), no CPUID/dispatch needed (SSE2 is Win64 baseline), producing a result bit-exact with the interpreters' `f64::sqrt`. It composes with the float lowering (`sqrt(a*a + b*b)`) and works on `f64` parameters routed through the XMM registers; verified by `native_sqrt_execution_parity_when_linkable` and the encoding-pinning unit test `compiles_sqrt_as_sqrtsd`. Still deferred: the remaining transcendental/math builtins (`sin`/`cos`/`floor`/`ceil`/`abs`/…) — `floor`/`ceil` need SSE4.1 `roundsd` behind a CPUID guard, the transcendentals need a libm call or a polynomial kernel — and `parse_f64`; a function using one still skips gracefully to the interpreters.

...
2. **`ptr_T` is a fully spellable type**, in a `let` annotation, a **parameter**, and
   ...
   **space-separated**, and a comma *groups same-type* parameters (`fn f x, y i64`) —
   ...
   `ptr_i64`.
3. **The two models do not interoperate at a boundary.** `let p ptr<i64> = alloc(8)`
   is `L0303` ("initializer has `ptr_i64`"), and passing an `alloc` pointer to a
   `ptr<i64>` parameter is `L0313`. There is no conversion between them. They are

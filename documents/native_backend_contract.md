...
**Default-deny.** `block_yields_value` requires that every path of a tail `if`/`match` provably yields the value (a non-void tail expression, a `return`, or a nested yielding `if`/`match`; an `if` chain must be exhaustive — a missing `else` lets control fall out with the value never routed). A tail that does not provably yield is refused with `L0339` and skips to the interpreters rather than emitting a function that returns whatever was already in the caller's buffer.

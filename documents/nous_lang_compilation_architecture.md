...
2. `nous_parser` builds an AST for functions, typed parameters, `let`, assignment, returns, break/continue, if/elif/else, while/loop blocks, calls, literals, variables, arithmetic, comparisons, and boolean logic.
...
4. `nous_runtime` executes the validated AST directly, including `main`, calls, scoped locals, assignment, branch result values, while/loop control flow, arithmetic/comparisons, short-circuit boolean logic, and heap-slot memory operations.

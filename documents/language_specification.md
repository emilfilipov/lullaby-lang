...
- `nlang compile [--optimize none|constant-fold|dead-code|alpha] [-o output.nbc] <file.nl>` emits a versioned `.nbc` bytecode artifact with metadata and a function table, `nlang inspect <file.nbc>` summarizes that artifact, and `nlang run <file.nbc>` executes it after compatibility checks.

...
- CLI commands are `nlang check [--verbose|--format json] <file.nl>` and `nlang run [--backend ast|ir|bytecode] [--optimize none|constant-fold|dead-code|alpha] [--verbose|--format json] <file.nl>` through the Rust workspace. `--diagnostic-format json` is also accepted as a JSON diagnostics alias.

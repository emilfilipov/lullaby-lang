...
- `nlang compile [--optimize none|constant-fold|dead-code|alpha] [-o output.nbc] <file.nl>` emits a versioned `.nbc` bytecode artifact with metadata and a function table, and `nlang run <file.nbc>` executes that artifact after compatibility checks.

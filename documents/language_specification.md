...
- `lullaby compile [--optimize none|constant-fold|dead-code|alpha] [-o output.lbc] <file.lullaby>` emits a versioned `.lbc` instruction-bytecode artifact with metadata, a function table, and memory operation metadata. `lullaby build` accepts the same artifact-generation options as an alias. `lullaby inspect <file.lbc>` summarizes that artifact, and `lullaby run <file.lbc>` executes it after compatibility checks.

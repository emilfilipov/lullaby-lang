    ...
    return [
        ...
        (
            ...
            <table>
              ...
              <tr><td><code>cargo run -p lullaby_cli -- compile --optimize alpha -o path/to/file.lbc path/to/file.lullaby</code><br><code>lullaby compile [--optimize none|constant-fold|dead-code|alpha] -o file.lbc file.lullaby</code></td><td>Validate executable source with zero-argument main, lower through typed IR, run the current alpha optimizer pipeline, and write a versioned <code>.lbc</code> instruction-bytecode artifact with metadata, a function table, memory operation metadata, and dedicated function instructions.</td></tr>
              ...
              <tr><td><code>cargo run -p lullaby_cli -- inspect path/to/file.lbc</code><br><code>lullaby inspect file.lbc</code></td><td>Print bytecode artifact metadata, function table details, memory operation counts, target, payload, entry point, and function count without executing the program.</td></tr>
        ...
        (
            ...
            <ul>
              ...
              <li>Version 4 <code>.lbc</code> artifacts preserve Alpha 1 memory operation metadata in <code>memory_operations</code>; the full region memory model, ARC/reference counting, compiler-inserted cleanup, and lifetime analysis remain planned.</li>

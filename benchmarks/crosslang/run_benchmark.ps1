...
#   3. assemble the self-contained HTML report   -> report.html
#
...
#   pwsh benchmarks/crosslang/run_benchmark.ps1 -Perf      # also re-run perf
#
[CmdletBinding()]
param(
    [switch]$Perf,
...
if ($Perf) {
    Write-Host "`n=== [2/3] running performance harness ===" -ForegroundColor Cyan
    & (Join-Path $cross 'run_perf.ps1') -Reps $Reps -PyReps $PyReps
} else {
    Write-Host "`n=== [2/3] skipping perf (pass -Perf to re-run) ===" -ForegroundColor DarkGray
}

...
- **Generated artifacts go in a `ScratchDir` — never a fixed `%TEMP%` path.** `crates/lullaby_cli/tests/cli/scratch.rs` mints a directory unique per process and per construction that removes itself on `Drop`. It is the harness's only sanctioned way to name a temp path: `let dir = ScratchDir::new("label"); let out = dir.join("thing.exe");`. It deliberately has **no `path()` accessor and no path-taking constructor** — if a call site seems to need one, restructure the call site. A helper that hands a path back to its caller must return the `ScratchDir` *with* it (see `fs_temp_dir`, `new_project_work_dir`), or the directory is deleted at the helper's closing brace. The CLI harness is **fully converted** (179 `%TEMP%` sites across 18 files, including `suite21.rs`; verified by 3 concurrent full suites at 519/519, zero `os error 32`). The only `std::env::temp_dir()` left in the whole harness is `scratch.rs`'s own inside `ScratchDir::new`; one anywhere else in these tests is a bug. **`%TEMP%` is not the only shared path**: `suite1.rs` also had 13 tests writing fixed artifacts into the workspace's own `target/` dir via `workspace_root().join("target/…")` — same disease, different directory, and the concurrent proof caught it (`os error 2` when one process deleted another's `.lbc` mid-read). Those are converted the same way (a `ScratchDir`, or a private cwd for the two fixtures that bake a relative `target/…` path into their `.lby`). The rule generalizes: **no test may write or read a fixed path any two processes can pick simultaneously**, whether under `%TEMP%` or under `target/`.
- **A fixed `%TEMP%` path does not just collide — it masks dead assertions.** `%TEMP%` is never cleaned, so an assertion reading a fixed path cannot distinguish this run's artifact from one left months ago. The conversion above caught four tests passing on stale files: three scanned a `.obj` the direct-PE fast path had stopped emitting (a 3-day-old file), and `ffi_calls_c_abs_when_linkable` executed a stale exe whose link step had silently skipped (it was the only linkable FFI test missing `ensure_msvc_env()`). All four were green while proving nothing. When an artifact-reading test starts failing after an isolation fix, suspect the assertion was dead, not the fix.
- **Do not misread parallel-load flakiness as a defect — or a defect as flakiness.** Re-run the affected suite **in isolation** before calling a failure real — and never dismiss one as "flaky" without proving it in isolation. Note that **thread count is a variable in its own right**: `ensure_msvc_env()` sets the process-global `LIB` from parallel test threads, so a test that omits it can pass at cargo's default thread count and fail at `--test-threads 4`. Reproduce with the *same* thread count before concluding anything. **The socket fixtures do not hardcode ports** — every bind in the CLI tests is `127.0.0.1:0` (ephemeral). The client-side TCP fixtures and all UDP fixtures hold their socket for the whole test and cannot race. The three fixtures where the *Lullaby program* is the server (`cli.rs` `tcp_server_round_trip`, `http_server_round_trip`, `suite2.rs:2032`) do carry a real but small race: they probe a free port by binding `:0`, drop it, then pass that number to the program to re-bind, so another process can take the port in the window. This is acknowledged in a comment at the probe. Closing it means having the program bind `:0` and report its port back, which needs streamed child stdout (`.output()` waits for exit and would deadlock) — a fixture redesign, not in the tree. Empirically it did not fire once across 3 concurrent full suites (1536 test executions).
...
- **The WASM backend has an in-process execution gate — run it, don't trust byte asserts alone.** `crates/lullaby_ir/src/wasm_exec_tests.rs` emits a module through the real pipeline, instantiates it under **`wasmi`** (a **pinned dev-dependency** of `lullaby_ir`: `wasmi = "=1.1.0"`, pure-Rust, in-process, deterministic — no Node, `wasm-pack`, or external toolchain) with the three stub host imports (`env.log_i64` / `console_log` / `dom_set_text`, matching the emitter), calls `main`, and asserts its `i64` return **equals the IR interpreter's** for the same source. It is the WASM analogue of the native execution-parity tests (compile+run a real `.exe`, check the exit code). Run it with `cargo test -p lullaby_ir --lib wasm::exec_tests`. `wasmi` is **dev-only** — never a runtime dependency of the compiler — and builds clean on both the host and the `x86_64-unknown-linux-gnu` cross-lint target. This gate exists because byte-only asserts (`wasm_tests.rs`) let a real cross-tier miscompile through: `let g = f` on a struct aliased `f`'s linear-memory pointer instead of value-copying, so mutating `g` corrupted `f` (WASM diverged from all three interpreters). Any new WASM value-semantics behavior gets an execution-parity case here, not just a byte assert.

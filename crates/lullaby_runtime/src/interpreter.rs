//! The AST interpreter: the `Runtime` evaluator, its lexical `Env`, the
//! `Control` flow signal, closure collection, the move-on-functional-update
//! analysis, and the `run_main`/`run_named_function` entry points. Split out of
//! `lib.rs` as a behavior-preserving code move. The builtin and expression/
//! statement halves of `impl Runtime` remain in the crate root's `builtins` and
//! `eval` submodules (which reach these types through `use super::*`), so the
//! items they touch are re-exported at the crate root. The value model and the
//! shared helper modules are reached through `use crate::*`.

...
fn fuzz_alloc_cast_launder_native_always_skips() {
    ...
    let dir = std::env::temp_dir().join("lullaby_fuzz_native_alloc_launder");
...
fn fuzz_alloc_cross_frame_native_matches_interpreter_when_linkable() {
    ...
    let dir = std::env::temp_dir().join("lullaby_fuzz_native_alloc_cross");
    let _ = std::fs::create_dir_all(&dir);
...
fn fuzz_alloc_arena_native_matches_interpreter_when_linkable() {
    ...
    let dir = std::env::temp_dir().join("lullaby_fuzz_native_alloc_arena");
    let _ = std::fs::create_dir_all(&dir);

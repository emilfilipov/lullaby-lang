...
pub(crate) fn ffi_calls_c_toupper_i32_when_linkable() {
    ...
    // Make MSVC's `LIB` available before the emit, like every other linkable FFI
    // test. Without it the CLI can emit obj-only (link unavailable), and then a
    // concurrent sibling test setting the process-global `LIB` makes the
    // `ucrt_available()` gate below demand an exe this run never linked — a race
    // the empty `ScratchDir` no longer masks with a stale image.
    ensure_msvc_env();

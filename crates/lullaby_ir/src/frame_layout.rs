    ...
    fn walk_block(&mut self, body: &[IrStmt], depth: usize, scope_names: &mut Vec<String>) {
        for stmt in body {
            match stmt {
                ...
                // `asm` DELIBERATELY does not descend into its operands here, and
                // neither does `Assign` into its target path: this walk records the
                // frame's SCOPE structure (which statements declare locals and open
                // child scopes), and neither an operand clause nor an `[i]` index
                // expression declares a binding or opens a scope — both only read
                // or write bindings the enclosing scope already laid out.
                IrStmt::Assign { .. }
                | IrStmt::Return(_)
                | IrStmt::Break(_)
                | IrStmt::Continue(_)

...
const UPDATE_ENV: &str = "NOUS_UPDATE_PARSER_SNAPSHOTS";
...
const CASES: &[SnapshotCase] = &[
    SnapshotCase {
        source: "tests/fixtures/valid/run_arithmetic.nl",
        snapshot: "tests/snapshots/run_arithmetic.ast.json",
    },
    SnapshotCase {
        source: "tests/fixtures/valid/run_array.nl",
        snapshot: "tests/snapshots/run_array.ast.json",
    },
    SnapshotCase {
        source: "tests/fixtures/valid/run_for_step.nl",
        snapshot: "tests/snapshots/run_for_step.ast.json",
    },
    SnapshotCase {
        source: "tests/fixtures/valid/docs_cleanup_helper.nl",
...
fn parser_ast_snapshots_match_checked_in_golden_files() {
    ...
    for case in CASES {
        ...
        assert_eq!(
            expected, actual,
            "parser AST snapshot changed for {}.\nReview the parser change, then refresh the checked-in golden files with PowerShell: `$env:NOUS_UPDATE_PARSER_SNAPSHOTS='1'; cargo test -p nous_parser --test ast_snapshots; Remove-Item Env:NOUS_UPDATE_PARSER_SNAPSHOTS`.",
            case.source
        );
    }
}
